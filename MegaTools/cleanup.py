...
"""
=============================================================================
QD Mega Tools - Maya 2022-2026 Compatible
=============================================================================
1) Mesh Toolkit v15 (Bridge / Detect / Vertices / Topology / Clean)  + toolkit réparé
2) PR Select Tools v3.8 (Angle select, inner faces, patterns, hard/border edges, etc.)
=> Les 2 outils sont intégrés dans UNE seule fenêtre avec 2 TABS.
=============================================================================
"""
...
def mesh_merge_vertices(threshold=0.001):
    ...
    try:
        total_merged = 0
        for mesh in meshes:
            initial = cmds.polyEvaluate(mesh, vertex=True)
            cmds.select("{}.vtx[*]".format(mesh), r=True)
            cmds.polyMergeVertex(distance=threshold, alwaysMergeTwoVertices=False)
            final = cmds.polyEvaluate(mesh, vertex=True)
            total_merged += (initial - final)

        cmds.select(meshes, r=True)
        show_inview_message("{} vertices fusionnés".format(total_merged), 2.0, "success" if total_merged > 0 else "info")
    except Exception as e:
...
class QDMegaToolsWindow(QtWidgets.QDialog):
    WINDOW_TITLE = "QD Mega Tools v15.1"
